...
class AdminController extends BaseController
{
    ...
    public function listMediaJson()
    {
        ...
        $allImages = $this->getImagesRecursive($uploadBase);
    ...
    private function getImagesRecursive($dir, $subDir = '')
    {
        $images = [];
        $files = scandir($dir);
        ...
        foreach ($files as $file) {
            ...
            $relativeDir = $subDir ? $subDir . '/' : '';
            
            if (is_dir($fullPath)) {
                $images = array_merge($images, $this->getImagesRecursive($fullPath, $relativeDir . $file));
            } else {
                $ext = strtolower(pathinfo($file, PATHINFO_EXTENSION));
                if (in_array($ext, $allowedExtensions)) {
                    $images[] = [
                        'name' => $file,
                        'url' => '/uploads/' . $relativeDir . $file,
    ...
    public function themes()
    {
        ...
        $res = $db->query("SELECT * FROM {$prefix}themes ORDER BY name ASC");
        $themes = [];
        if ($res) {
            while ($row = $res->fetch_assoc()) {
                $themes[] = $row;
    ...
    public function editTheme($id)
    {
        ...
        if ($_SERVER['REQUEST_METHOD'] === 'POST') {
            ...
            // Build the settings JSON from POST array
            $colors = [
                'primary' => $_POST['color_primary'] ?? '#1e40af',
                'secondary' => $_POST['color_secondary'] ?? '#6b7280',
                'accent' => $_POST['color_accent'] ?? '#3b82f6',
                'text' => $_POST['color_text'] ?? '#111827',
                'link' => $_POST['color_link'] ?? '#2563eb',
                'header_bg' => $_POST['color_header_bg'] ?? '#ffffff',
                'header_text' => $_POST['color_header_text'] ?? '#111827',
                'footer_bg' => $_POST['color_footer_bg'] ?? '#1f2937',
                'footer_text' => $_POST['color_footer_text'] ?? '#f9fafb'
            ];
            
            $background = [
                'type' => $_POST['bg_type'] ?? 'color',
                'value' => $_POST['bg_value'] ?? '#f3f4f6',
                'image' => $_POST['bg_image'] ?? ''
            ];
            
            $typography = [
                'body_font' => $_POST['font_body'] ?? 'Inter, sans-serif',
                'heading_font' => $_POST['font_heading'] ?? 'Inter, sans-serif',
                'h1' => ['size' => $_POST['font_h1_size'] ?? '2.5rem', 'line_height' => $_POST['font_h1_lh'] ?? '1.2'],
                'h2' => ['size' => $_POST['font_h2_size'] ?? '2rem', 'line_height' => $_POST['font_h2_lh'] ?? '1.2'],
                'h3' => ['size' => $_POST['font_h3_size'] ?? '1.75rem', 'line_height' => $_POST['font_h3_lh'] ?? '1.2'],
                'p' => ['size' => $_POST['font_p_size'] ?? '1rem', 'line_height' => $_POST['font_p_lh'] ?? '1.5']
            ];
            
            $spacing = [
                'section_padding' => $_POST['spacing_section'] ?? '4rem 0',
                'container_width' => $_POST['spacing_container'] ?? '1200px',
                'border_radius' => $_POST['spacing_radius'] ?? '0.5rem'
            ];
            ...
            if (empty($name) || empty($slug)) {
                ...
                $slugCheck = $db->query("SELECT id FROM {$prefix}themes WHERE slug = '{$db->real_escape_string($slug)}' AND id != $id");
                if ($slugCheck && $slugCheck->num_rows > 0) {
                    $error = "Slug is al in gebruik.";
                } else {
                    $stmt = $db->prepare("UPDATE {$prefix}themes SET name = ?, slug = ?, settings_json = ? WHERE id = ?");
                    $stmt->bind_param("sssi", $name, $slug, $settingsJson, $id);
                    if ($stmt->execute()) {
                        $success = "Thema is succesvol geüpdatet.";
                    } else {
        ...
        $res = $db->query("SELECT * FROM {$prefix}themes WHERE id = $id");
    ...
    public function deleteTheme($id)
    {
        ...
        if ($check && $row = $check->fetch_assoc()) {
            ...
            if ($row['is_active']) {
                header('Location: /backoffice/themes?error=cannot_delete_active');
                exit;
            }
        }
